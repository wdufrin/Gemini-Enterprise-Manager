...
const InnerApp: React.FC = () => {
  ...
  const [architectureElapsedSeconds, setArchitectureElapsedSeconds] = useState(0);
  ...
  // Listen for in-flight 401s / auth expired events from apiService
  useEffect(() => {
    const unsubscribe = api.onAuthExpired(() => {
      console.warn("Authentication expired received from apiService.");
  ...
  // Track elapsed seconds during architecture scan
  useEffect(() => {
    let interval: any = null;
    if (isArchitectureLoading) {
      interval = setInterval(() => {
        setArchitectureElapsedSeconds(prev => prev + 1);
      }, 1000);
    }
    return () => {
  ...
  const handleCancelArchitectureScan = useCallback(() => {
    if (architectureAbortRef.current) {
      architectureAbortRef.current.abort();
    }
    setIsArchitectureLoading(false);
    setArchitectureLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] Architecture scan cancelled by user.`]);
  }, []);
  ...
  const handleArchitectureScan = useCallback(async () => {
        ...
        architectureAbortRef.current = new AbortController();
        const signal = architectureAbortRef.current.signal;
        setArchitectureElapsedSeconds(0);
    ...
    switch (currentPage) {
      ...
      case Page.ARCHITECTURE:
        return <ArchitecturePage 
                    ...
                    onCancelScan={handleCancelArchitectureScan}
                    elapsedSeconds={architectureElapsedSeconds}
                />;
      default:
          ...
          <div className="bg-gray-800 border border-amber-500/40 rounded-xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Re-authenticate to continue without losing your current work or page state.</p>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="relative flex py-1 items-center">
                <div className="flex-grow border-t border-gray-700"></div>
                <span className="flex-shrink mx-3 text-gray-500 text-xs uppercase">Or paste access token</span>
                <div className="flex-grow border-t border-gray-700"></div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  onClick={() => {
                    handleSetAccessToken(reauthTokenInput.trim());
                    setReauthTokenInput('');
                    setShowReauthModal(false);
                  }}
