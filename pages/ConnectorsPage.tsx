  ...
  const checkDataConnector = async (collection: Collection) => {
    ...
    try {
      ...
      const failures = recentOps.filter((op: any) => {
        ...
        const failureCount = op.metadata?.failureCount
          ? parseInt(op.metadata.failureCount, 10)
          : 0;
        const hasImportErrors =
          failureCount > 0 ||
          (op.response?.errorSamples && op.response.errorSamples.length > 0);

        const isRecent =
          new Date(
            op.metadata?.createTime || op.metadata?.updateTime || Date.now(),
          ).getTime() >
          Date.now() - 86400000;
      ...
      if (failures.length > 0) {
        ...
        failures.forEach((op: any) => {
          const errMsg =
            op.error?.message ||
            (op.response?.errorConfig
              ? `Import errors written to GCS prefix: ${op.response.errorConfig.gcsPrefix}`
              : "Operation failed");
      ...
      if (connector && connector.dataSource === "custom_mcp") {
        const mcpEndpointUrl =
          connector.params?.instance_uri ||
          connector.actionConfig?.actionParams?.instance_uri;
        if (mcpEndpointUrl) {
          ...
          try {
            ...
            addStep(
              "Verify MCP Connectivity",
              "fail",
              e.message || "Failed to connect to MCP endpoint",
            );
            status = "error";
      ...
      const duration =
        typeof scanDurationHours === "number" ? scanDurationHours : 2;
      const mcpEndpointUrl =
        connector.params?.instance_uri ||
        connector.actionConfig?.actionParams?.instance_uri;
