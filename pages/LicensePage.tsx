...
const LicensePage: React.FC<LicensePageProps> = ({ projectNumber, setProjectNumber, onBuildTriggered }) => {
  ...
  const fetchUserLicenses = async (forceRefresh: boolean = false) => {
      ...
      try {
          // Check browser cache first unless user explicitly forced a refresh
          if (!forceRefresh) {
          ...
          do {
              // Fetch maximum allowed page size (1000) to minimize requests
              const result: any = await api.listUserStoreLicenses(config, apiConfig.userStoreId, userLicensesFilter, nextPageToken, 1000);
  ...
  const confirmSingleDelete = async () => {
      ...
      try {
          ...
          if (actionType === 'delete') {
              ...
              // Optimistic update: Remove from local state and cache immediately
              const filterFn = (prev: any[]) => prev.filter(l => l.userPrincipal !== userPrincipal);
              ...
              // Optimistic update: Keep user but set as UNASSIGNED in local state and cache
              const updateFn = (prev: any[]) => prev.map(l => 
  ...
  const handlePrune = async (days: number) => {
      ...
      try {
          ...
          if (principals.length > 0) {
              ...
              const filterFn = (prev: any[]) => prev.filter(l => !principalSet.has(l.userPrincipal));
              setUserLicenses(filterFn);
              await updateCachedUserLicenses(projectNumber, apiConfig.userStoreId, filterFn);
          }
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-1">
                    ...
                    {cacheTimestamp && !isLicensesLoading && (
                        <div className="inline-flex items-center gap-2 text-xs text-gray-300 bg-gray-700/60 border border-gray-600/60 px-3 py-1 rounded-full w-fit">
                            ...
                            <span>Cached ({userLicenses.length.toLocaleString()} users) &bull; Synced {new Date(cacheTimestamp).toLocaleTimeString()}</span>
                            <button
                                type="button"
