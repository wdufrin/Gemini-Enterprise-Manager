...
import { Config, RegistrySkill, RegistrySkillRevision, AppEngine, Agent } from '../../types';
...
const RegistrySkillDetailModal: React.FC<RegistrySkillDetailModalProps> = ({
  ...
  const [availableEngines, setAvailableEngines] = useState<AppEngine[]>([]);
  ...
  useModalA11y({
    ...
    onClose,
    containerRef,
    preventClose: isUpdating || isDeleteConfirmOpen,
  });
  ...
  const handleDeployToGeEngine = async () => {
    ...
    try {
      ...
      try {
        const activeRev = revisions.find((r: RegistrySkillRevision) => r.name === skill.defaultRevision || r.state === 'ACTIVE') || revisions[0];
        const archiveContent = activeRev?.archiveUploadSource?.archiveContent ||
          skill.initialRevision?.archiveUploadSource?.archiveContent;
        if (archiveContent) {
      ...
      if (selectedEngineId) {
        ...
        const enginesRes = await api.listResources<AppEngine>('engines', config);
        const engines = enginesRes.engines || enginesRes.resources || [];
        if (engines.length === 0) {
        ...
        const targetEngine = engines.find((e: AppEngine) => e.solutionType === 'SOLUTION_TYPE_CHAT' || e.name?.includes('cosmere')) || engines[0];
        const engineId = targetEngine.name ? targetEngine.name.split('/').pop() : (targetEngine.id as string | undefined);
...
${skill.description || 'Enterprise Skill for Gemini Enterprise.'}`);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="registry-skill-detail-title"
      onClick={(e) => {
        if (e.target === e.currentTarget && !isUpdating && !isDeleteConfirmOpen) onClose();
      }}
    >
      <div
        ref={containerRef}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 id="registry-skill-detail-title" className="text-base font-bold text-gray-100">{skill.displayName}</h3>
          ...
          <div className="flex items-center gap-2">
            {availableEngines.length > 0 && (
              <div className="flex items-center bg-gray-900 border border-gray-700 rounded-lg px-2 py-1 max-w-[210px]" title="Target Gemini Enterprise App">
                ...
                >
                  {availableEngines.map((eng: AppEngine) => {
                    const id = eng.name ? eng.name.split('/').pop() : (eng.id as string | undefined);
