...
export const SYSTEM_CURL_COMMANDS: CurlInfoMap = {
  [Page.ASSISTANT]: {
...
curl -X GET \\
  ...
  [Page.AGENT_PERMISSIONS]: {
    description: "These are the REST API calls for inspecting and modifying IAM policies on individual agents within Gemini Enterprise.",
    commands: [
      {
        title: 'Get Agent IAM Policy',
        command: `curl -X GET \\
  -H "Authorization: Bearer [YOUR_ACCESS_TOKEN]" \\
  -H "X-Goog-User-Project: [YOUR_PROJECT_ID]" \\
  "https://discoveryengine.googleapis.com/v1alpha/projects/[YOUR_PROJECT_ID]/locations/[LOCATION]/collections/default_collection/engines/[ENGINE_ID]/assistants/[ASSISTANT_ID]/agents/[AGENT_ID]:getIamPolicy"`
      },
      {
        title: 'Set Agent IAM Policy',
        command: `curl -X POST \\
  -H "Authorization: Bearer [YOUR_ACCESS_TOKEN]" \\
  -H "Content-Type: application/json" \\
  -H "X-Goog-User-Project: [YOUR_PROJECT_ID]" \\
  ...
  "https://discoveryengine.googleapis.com/v1alpha/projects/[YOUR_PROJECT_ID]/locations/[LOCATION]/collections/default_collection/engines/[ENGINE_ID]/assistants/[ASSISTANT_ID]/agents/[AGENT_ID]:setIamPolicy"`
      }
    ]
  },
  [Page.OBSERVABILITY]: {
    description: "These are the REST API calls for operational analytics, BigQuery query execution, and logging sink inspections.",
    commands: [
      {
        title: 'Query Operational Analytics (BigQuery)',
        command: `curl -X POST \\
  -H "Authorization: Bearer [YOUR_ACCESS_TOKEN]" \\
  -H "Content-Type: application/json" \\
  -d '{
        "query": "SELECT * FROM \`[YOUR_PROJECT_ID].[DATASET_ID].v_consolidated_user_activity\` LIMIT 100;",
        "useLegacySql": false
      }' \\
  "https://bigquery.googleapis.com/bigquery/v2/projects/[YOUR_PROJECT_ID]/queries"`
      },
      {
        title: 'List Cloud Logging Sinks',
        command: `curl -X GET \\
  -H "Authorization: Bearer [YOUR_ACCESS_TOKEN]" \\
  "https://logging.googleapis.com/v2/projects/[YOUR_PROJECT_ID]/sinks"`
      }
    ]
  },
  [Page.BACKUP_RECOVERY]: {
    description: "These are the REST API calls for backing up and restoring Gemini Enterprise configuration snapshots to and from Cloud Storage.",
    commands: [
      {
  ...
  "https://storage.googleapis.com/storage/v1/b?project=[YOUR_PROJECT_ID]"`
      ...
      {
        title: 'List Backup Snapshots in Bucket',
        command: `curl -X GET \\
  -H "Authorization: Bearer [YOUR_ACCESS_TOKEN]" \\
  "https://storage.googleapis.com/storage/v1/b/[BUCKET_NAME]/o"`
      },
      {
        ...
        command: `curl -X GET \\
  -H "Authorization: Bearer [YOUR_ACCESS_TOKEN]" \\
  "https://storage.googleapis.com/storage/v1/b/[BUCKET_NAME]/o/[BACKUP_FILENAME]?alt=media"`
      }
    ]
  },
  [Page.CONFIG_AUDIT]: {
    description: "These are the REST API calls for auditing project configuration, active APIs, and IAM service agents.",
    commands: [
      {
        title: 'Get Project Metadata',
        command: `curl -X GET \\
  -H "Authorization: Bearer [YOUR_ACCESS_TOKEN]" \\
  "https://cloudresourcemanager.googleapis.com/v1/projects/[YOUR_PROJECT_ID]"`
      },
      {
        title: 'List Enabled Service APIs',
        command: `curl -X GET \\
  -H "Authorization: Bearer [YOUR_ACCESS_TOKEN]" \\
  "https://serviceusage.googleapis.com/v1/projects/[YOUR_PROJECT_ID]/services?filter=state:ENABLED"`
      },
      {
